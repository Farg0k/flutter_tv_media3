...
typedef OnScreenshotTaken = Future<void> Function({required Uint8List bytes, required PlaylistMediaItem item});
...
class FtvMedia3PlayerController {
  ...
  String _formatErrorMessage(dynamic error) {
    ...
    if (error is PlatformException) {
      return error.message ?? OverlayLocalizations.get('linkResolutionUnknownError');
    }

    final msg = error.toString().replaceAll("Exception: ", "");
    return msg.isNotEmpty ? msg : OverlayLocalizations.get('linkResolutionUnknownError');
  }
  ...
  Future<dynamic> _handleMethodCall(MethodCall call) async {
    ...
    switch (call.method) {
          ...
          if (item.getDirectLink != null) {
            ...
            try {
              ...
              if (MediaRequestManager.isCurrentRequest(requestId)) {
                ...
                _sendLoadProgressToUI(
                  state: "${OverlayLocalizations.get('errorPrefix')} $errorMessage",
                  progress: null,
                  requestId: requestId,
                );
      ...
      case 'onScreenshotTaken':
        ...
        if (bytes != null && playlistIndex != null && _onScreenshotTaken != null) {
      ...
      case 'onItemRemoved':
        ...
        if (playlistStr != null) {
          ...
          final currentPlaylist = playlistList.map((e) => PlaylistMediaItem.fromMap(e)).toList();
          newState = newState.copyWith(playlist: currentPlaylist, playIndex: playIndex ?? newState.playIndex);
        }
        _updateState(newState);
        break;
  ...
  Future<void> openNativePlayer({required List<PlaylistMediaItem> playlist, int initialIndex = 0}) async {
    ...
    try {
      await _invokeMethodGuarded<void>(_pluginChannel, 'openPlayer', {
        ...
        "stuck_buffering_detection_timeout_ms": _playerSettings.stuckBufferingDetectionTimeoutMs,
        "stuck_playing_detection_timeout_ms": _playerSettings.stuckPlayingDetectionTimeoutMs,
        "stuck_playing_not_ending_timeout_ms": _playerSettings.stuckPlayingNotEndingTimeoutMs,
        "stuck_suppressed_detection_timeout_ms": _playerSettings.stuckSuppressedDetectionTimeoutMs,
      });
  ...
  Future<Uint8List?> getVideoThumbnail(String uri, {double? timeInSeconds}) async {
    final Map<String, dynamic> arguments = {'uri': uri};
    if (timeInSeconds != null && timeInSeconds >= 0) {
      arguments['timeInSeconds'] = timeInSeconds;
    }
    final Uint8List? result = await _invokeMethodGuarded(_pluginChannel, 'getThumbnail', arguments);
    return result;
  }
  ...
  Future<Map<String, dynamic>?> getMediaMetadata(String uri) async {
    final dynamic result = await _invokeMethodGuarded(_pluginChannel, 'getMetadata', {'uri': uri});
  ...
  static Map<String, dynamic>? _castMap(dynamic data) {
    ...
    if (data is Map) {
      return data.map((key, value) {
        ...
        if (value is List) {
          return MapEntry(stringKey, value.map((e) => e is Map ? _castMap(e) : e).toList());
        }
  ...
  Future<void> setRepeatMode({required PlayerRepeatMode repeatMode}) async {
    try {
      ...
      _updateState(_playerState.copyWith(repeatMode: PlayerRepeatMode.fromString(repeat)));
    } on PlatformException catch (e) {
      _updateState(_playerState.copyWith(lastError: e.message ?? 'Unknown platform error', errorCode: e.code));
    } catch (e) {
  ...
  Future<void> addMediaItems({required List<PlaylistMediaItem> items}) async {
    final updatedPlaylist = List<PlaylistMediaItem>.from(_playerState.playlist)..addAll(items);
  ...
  Future<void> removeMediaItem({required int index}) async {
    ...
    final updatedPlaylist = List<PlaylistMediaItem>.from(_playerState.playlist)..removeAt(index);
    _updateState(_playerState.copyWith(playlist: updatedPlaylist));
